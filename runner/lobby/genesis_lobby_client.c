...
int  genesis_lobby_connect(const char *ws_url) { (void)ws_url; return -1; }
void genesis_lobby_disconnect(void) {}
int  genesis_lobby_connected(void) { return 0; }
void genesis_lobby_set_display_name(const char *name) { (void)name; }
const char *genesis_lobby_display_name(void) { return ""; }
const char *genesis_lobby_player_id(void) { return ""; }
void genesis_lobby_pump(void) {}
void genesis_lobby_request_list(void) {}
int  genesis_lobby_list_count(void) { return 0; }
int  genesis_lobby_list_get(int index, GenesisLobbyRow *out) { (void)index; (void)out; return 0; }
void genesis_lobby_set_game_identity(const char *a, const char *b) { (void)a; (void)b; }
const char *genesis_lobby_game_version(void) { return GENESIS_GAME_VERSION; }
int  genesis_lobby_create(const char *a, const char *b, const char *c, const char *d,
                       const char *e, const GenesisLobbyMatchCaps *f)
{ (void)a; (void)b; (void)c; (void)d; (void)e; (void)f; return -1; }
int  genesis_lobby_join(const char *a, const char *b, const char *c)
{ (void)a; (void)b; (void)c; return -1; }
int  genesis_lobby_leave(void) { return -1; }
int  genesis_lobby_in_lobby(void) { return 0; }
int  genesis_lobby_is_host(void) { return 0; }
const GenesisLobbyJoinInfo *genesis_lobby_join_info(void)
{
    static GenesisLobbyJoinInfo z;
    return &z;
}
const GenesisLobbyMatchCaps *genesis_lobby_match_caps(void)
{
    static GenesisLobbyMatchCaps z;
    return &z;
}
int  genesis_lobby_set_match_caps(const GenesisLobbyMatchCaps *c) { (void)c; return -1; }
int  genesis_lobby_member_count(void) { return 0; }
int  genesis_lobby_member_get(int index, GenesisLobbyMember *out) { (void)index; (void)out; return 0; }
int  genesis_lobby_local_ready(void) { return 0; }
int  genesis_lobby_all_ready(void) { return 0; }
int  genesis_lobby_set_ready(int ready) { (void)ready; return -1; }
int  genesis_lobby_request_start(const GenesisLobbyMatchCaps *c) { (void)c; return -1; }
int  genesis_lobby_launch_pending(void) { return 0; }
...
typedef struct {
    ...
    GenesisLobbyRow list[GENESIS_LOBBY_MAX_LIST];
    int list_count;
    int in_lobby;
    int is_host;
    char my_bind[GENESIS_LOBBY_ENDPOINT_LEN];
    char filter_game_name[GENESIS_LOBBY_NAME_LEN];
    char filter_game_version[GENESIS_LOBBY_VERSION_LEN];
    GenesisLobbyJoinInfo join;
    GenesisLobbyMember members[GENESIS_LOBBY_MAX_MEMBERS];
    int member_count;
    int local_ready;
    int all_ready;
    int launch_pending;
    GenesisLobbyMatchCaps match_caps;
...
static LobbyClient g_lc = {
    .fd = -1,
    .filter_game_version = GENESIS_GAME_VERSION,
};
...
static void match_caps_clear(GenesisLobbyMatchCaps *c)
{
    if (!c) return;
    memset(c, 0, sizeof(*c));
    c->input_delay = 2;
...
static void parse_match_caps_object(const char *obj, GenesisLobbyMatchCaps *out);
static void ingest_match_caps_from_json(const char *json);
static int append_match_caps_json(char *dst, size_t dst_cap, const GenesisLobbyMatchCaps *caps);

const char *genesis_lobby_default_url(void)
{
    const char *e = getenv("GENESIS_NET_LOBBY_URL");
...
static void parse_match_caps_object(const char *obj, GenesisLobbyMatchCaps *out)
{
    ...
    out->widescreen_cells = json_get_int(obj, "widescreen_cells", 0);
    out->pad_mode[0] = json_get_int(obj, "pad_mode_p1", 0) ? 1 : 0;
    out->pad_mode[1] = json_get_int(obj, "pad_mode_p2", 0) ? 1 : 0;
...
static int append_match_caps_json(char *dst, size_t dst_cap, const GenesisLobbyMatchCaps *caps)
{
    ...
    return snprintf(dst, dst_cap,
                    ",\"match_caps\":{\"v\":1,\"widescreen\":%s,"
                    "\"widescreen_cells\":%d,\"pad_mode_p1\":%d,"
                    "\"pad_mode_p2\":%d,\"input_delay\":%d}",
                    caps->widescreen ? "true" : "false",
                    caps->widescreen_cells,
                    caps->pad_mode[0] ? 1 : 0,
                    caps->pad_mode[1] ? 1 : 0,
                    caps->input_delay);
}
...
{
    GenesisLobbyJoinInfo *j = &g_lc.join;
    const char *port;
...
static void parse_slots_array(const char *json)
{
    ...
    ++p;
    while (*p && n < GENESIS_LOBBY_MAX_MEMBERS) {
        const char *obj;
        while (*p && *p != '{') {
            if (*p == ']') {
                g_lc.member_count = n;
...
static void handle_server_json(const char *json)
{
    ...
    if (strcmp(op, "lobby_list") == 0) {
        ...
        ++p;
        while (*p && n < GENESIS_LOBBY_MAX_LIST) {
            const char *obj;
            while (*p && *p != '{') {
                if (*p == ']') {
                    g_lc.list_count = n;
...
int genesis_lobby_connect(const char *ws_url)
{
...
#endif
    if (parse_ws_url(ws_url ? ws_url : genesis_lobby_default_url(), g_lc.host, sizeof(g_lc.host),
...
void genesis_lobby_disconnect(void)
{
    ...
    {
        char dname[GENESIS_LOBBY_NAME_LEN];
                                  ...
                                  const char *game_version)
{
    ...
    if (game_version && game_version[0]) {
        ...
        strncpy(g_lc.filter_game_version, GENESIS_GAME_VERSION,
                sizeof(g_lc.filter_game_version) - 1);
        g_lc.filter_game_version[sizeof(g_lc.filter_game_version) - 1] = '\0';
    }
}
...
int genesis_lobby_create(const char *name, const char *game_name,
                     ...
                     const char *host_bind, const GenesisLobbyMatchCaps *match_caps)
{
    char msg[1536];
    char caps_json[512];
    const char *gn;
    ...
    if (game_name && game_name[0])
        genesis_lobby_set_game_identity(game_name, gv);
...
int genesis_lobby_join(const char *lobby_id, const char *password, const char *guest_bind)
{
    char msg[1024];
    const char *gn;
    const char *gv;
    if (!genesis_lobby_connected() || !lobby_id) {
        return -1;
    }
...
int genesis_lobby_set_match_caps(const GenesisLobbyMatchCaps *caps)
{
    ...
    char caps_json[512];
    int n;
    if (!genesis_lobby_connected() || !g_lc.in_lobby || !g_lc.is_host || !caps || !caps->valid)
        return -1;
...
{
    char msg[64];
    if (!genesis_lobby_connected() || !g_lc.in_lobby) {
        return -1;
    }
...
int genesis_lobby_request_start(const GenesisLobbyMatchCaps *match_caps)
{
    ...
    char caps_json[512];
    int n;
    if (!genesis_lobby_connected() || !g_lc.in_lobby || !g_lc.is_host) {
        return -1;
    }
    caps_json[0] = '\0';
    if (match_caps && match_caps->valid) {
...
int genesis_lobby_send_signal(int type, int flag, const char *text)
{
    ...
    snprintf(msg, sizeof(msg),
             ...
             lid, type, flag, esc);
    /* Write immediately Ã¢â‚¬â€ ICE candidates arrive in bursts larger than pending_tx. */
...
#endif /* GENESIS_HAS_LOBBY_CLIENT */
